...
def parallelize_clusters(phio, reqarr, save_plots=False, minor='depth', 
                         nclusters=300, ncrystals=2, numaspectratios=20, speedy=True, rand_orient=False,
                         ch_dist='gamma'):   
    #counter_lock = Lock()

    c_n = np.zeros(shape=(len(reqarr)))
    a_n = np.zeros(shape=(len(reqarr)))
    a_avg = np.zeros(shape=(len(reqarr)))
    dd = np.zeros(shape=(len(reqarr)))
    phioarr = []
    #phi_ind = 0
    #for i in phio:
    ...
    for r in reqarr:   #array of equivalent volume radii to loop through
        ...
        print('phi, req, c, a =', phio, r, length, width)
        ...
        if phio < 1:  #build the lookup table 2D arrays, i loops over aspect ratio
            #have to switch a and c for plates vs. columns           
            c_n[req_ind] = ch_depth
            a_n[req_ind] = ch_majorax
            a_avg[req_ind] = mean_mjrax
            dd[req_ind] = ch_dd

        else:
            c_n[req_ind] = ch_majorax
            a_n[req_ind] = ch_depth
            a_avg[req_ind] = mean_depth
            dd[req_ind] = ch_dd
