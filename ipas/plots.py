...
class Make_Plots():
    ...
    def __init__(self, phio, widtharr, lengtharr, chreq, chphi, ch_ovrlp, ch_S, ovrlp, S, ch_majorax, ch_depth, dphigam, 
    ...
    def which_plot(self,nclusters,plot_name, ch_dist, savefile, read_file='outfile.dat', save=False, verbose=False, ext='png'):   
        ...
        if plot_name == 'overlap':
            ...
            ovrlps = [np.squeeze(np.array(self.ovrlp[i,:])) for i in range(self.ovrlp.shape[0])]
        ...
        if plot_name == 'S':   
            ...
            xstar = np.arange(0,50)
            plt.plot(xstar,self.ch_S,'*',color='blue',markersize=6, label='characteristic')
            #plt.plot(phiolog,self.ch_ovrlp,color='navy')
           
            #ax.set_xscale("log")
            print(np.shape(self.ovrlp))
            print(np.shape(self.S[0,:]))
        ...
        if plot_name == 'max_contact_angle':
            ...
            plt.plot(self.phio,randrot,'--',color='darkorange')
