...
#from crystal_dda import geometry as geo
...
class IceCrystal:
    ...
    def __init__(self, length, width, center=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        ca = length*2 # c axis diameter from equiv. volume radius in lab module
        mf = width*2 # maximum face dimension
        ...
        if length < width:  #initialize plates so that the basal face is falling down
            ...
            amax = width/2 #max a-axis length of the crystal
            ac = 0.3  #a-axis length of the core
            ...
            self.points = np.array([(x[i],y[i],x1) for i in range(len(x))],                                   
                       dtype=[('x', float), ('y', float), ('z', float)])
            '''
            self.pointsbot = np.array([(x[i],y[i],-x1) for i in range(len(x))],                                   
                       dtype=[('x', float), ('y', float), ('z', float)])
            self.points = np.concatenate((self.pointstop,self.pointsbot))
    ...
    def plot(self):
        ...
        #    lines.append(geom.LineString([(self.pointstop['x'][n], self.pointstop['y'][n], self.pointstop['z'][n]),
        #                                 (self.pointsbot['x'][n], self.pointsbot['y'][n],self.pointsbot['z'][n])]))
        '''
        for n in range(len(self.pointsbot['x'])-1):    
            lines.append(geom.LineString([(self.pointsbot['x'][n], self.pointsbot['y'][n], self.pointsbot['z'][n]),
                                         (self.pointsbot['x'][n+1], self.pointsbot['y'][n+1],self.pointsbot['z'][n+1])]))
        '''
        for n in range(len(self.points['x'])-1):    
            lines.append(geom.LineString([(self.points['x'][n], self.points['y'][n], self.points['z'][n]),
                                         (self.points['x'][n+1], self.points['y'][n+1],self.points['z'][n+1])]))
...
class IceCluster:
    ...
    #Calculate separation of crystals for further collection restriction        
        ...
        xclus, yclus = list(self.projectxy().exterior.coords.xy)
        start = Point(xclus[0],yclus[0])
        d = []
        for i in range(len(xclus)):
            d.append(start.distance(Point(xclus[i],yclus[i])))  
        maxind = np.argmax(d)
        dmaxclus = max(d)     
        end = Point(xclus[maxind],yclus[maxind])

        #max distance of new crystal  
        xnew, ynew = list(crystal.projectxy().exterior.coords.xy)
        start = Point(xnew[0],ynew[0])
        d = []
        for i in range(len(xnew)):
            d.append(start.distance(Point(xnew[i],ynew[i])))  
        maxind = np.argmax(d)
        dmaxnew = max(d)   
        end = Point(xnew[maxind],ynew[maxind])
    ...
    def add_flow_tilt(self, crystal, lmax):
        ...
        print('fracx',tilt_fracx)
        print('fracy',tilt_fracy)
    ...
    def plot_ellipse(self, dims):
        ...
        #self.reorient(method=reorient, rotations=rotations)
        # get the ellipse
        ...
        for l in range(len(dims)):
            ...
            for i in range(self.ncrystals):    
                ...
                for n in range(7):  #plot the prism face lines
                    x = [x1[n],x2[n]]
                    y = [y1[n],y2[n]]
                    ax.plot(x,y, color = color, zorder = 3, linewidth = '2')
                ...
                ax.plot(x2,y2, color = color, zorder = 2, linewidth = '2') 
