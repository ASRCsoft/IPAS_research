...
def sim_clusters(r, length, width, nclusters, ncrystals=2, numaspectratios=1,
        ...
        while cluster.ncrystals < ncrystals:  
            ...
            if crystal_hit:
                ...
                #                         xmaxseed, xminseed, xmaxnew, xminnew)
                    ...
                    #    cluster.rotate_to([x[0], yrot, 0])
                    #    cluster.rotate_to([x[0], x[1], 0])
                    ...
                    #cluster.rotate_to(rotation)
                    #print('area = ', cluster.projectxy().area)
                ...
                cluster.reorient(method=reorient, rotations=rotations)
                    ...
                    #print('area = ', cluster.projectxy().area)
                    #cluster.reorient(method=reorient, rotations=rotations)
                    #print('second area = ', cluster.projectxy().area)
    ...
    return IceClusterBatch(clusters, length, width, r, numaspectratios, reorient, ovrlps, cplxs, nclusters,
                           xrot, yrot, plates)


def overlap(seedcrystal, new_crystal, cluster):

    rel_area = cluster.projectxy().intersection(new_crystal.projectxy())
...
class IceClusterBatch():
    ...
    def __init__(self, clusters, length, width, r, numaspectratios, reorient, ovrlps, cplxs, nclusters, 
