...
class Make_Plots():
    ...
    def __init__(self, phio, widtharr, lengtharr, chreq, chphi, ch_ovrlp, ch_majorax, ch_depth, dphigam, 
                 poserr_phi, negerr_phi, poserr_mjrax, negerr_mjrax, poserr_req, negerr_req, poserr_depth, 
                 negerr_depth, poserr_ovrlp, negerr_ovrlp, poserr_cplx, negerr_cplx, min_phi, max_phi, 
                 min_mjrax, max_mjrax, min_depth, max_depth, min_req, max_req, mean_phi, mean_mjrax, mean_depth, 
                 mean_req, mean_ovrlp, mean_cplx, ch_cplx, xrot, yrot):
    ...
    def which_plot(self, nclusters, plot_name, ch_dist, savefile, read_file='outfile.dat', save=False, verbose=False, ext='png'):   
        ...
        reqp = np.power((np.power(self.widthp,2)*self.lengthp),(1./3.))
        reqc = np.power((np.power(self.widthc,2)*self.lengthc),(1./3.))
        ...
        if plot_name == 'char':   
            ...
            plt.plot(reqp, self.chp,'o',color='navy',markersize=6)
            plt.plot(reqp, self.chp, color='navy')
            ...
            plt.plot(reqc, self.chc,'o',color='navy',markersize=6)
            plt.plot(reqc, self.chc, color='navy', label = 'characteristic')
            #plt.plot(self.phio, self.mean_phi,'o',color='black',markersize=6)
      
            plt.plot(reqp, self.meanphip, color='black')
            plt.plot(reqc, self.meanphic, color='black', label = 'mean')
            ax.fill_between(reqp, poserr_phip, negerr_phip, facecolor='darkorange', alpha=0.3)
            ax.fill_between(reqc, poserr_phic, negerr_phic, facecolor='darkorange', alpha=0.3)
        ...
        if plot_name == 'overlap':
            ...
            ax.fill_between(self.phio, self.poserr_ovrlp/self.mean_ovrlp, self.negerr_ovrlp/self.mean_ovrlp, facecolor='darkorange', alpha=0.5)           
