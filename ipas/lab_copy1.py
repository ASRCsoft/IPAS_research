...
def main_ar_loop(phio, reqarr, numaspectratios, ch_dist, nclusters, ncrystals, minor, save_plots, file_ext):
    ...
    ovrlp4 = []
    ovrlp5 = []
    c_n = np.zeros(shape=(len(phio),len(reqarr)))
    a_n = np.zeros(shape=(len(phio),len(reqarr)))
    a_avg = np.zeros(shape=(len(phio),len(reqarr)))
    ...
    for i in phio:
        ...
        req_ind = 0
        for r in reqarr:
            width = (r**3/i)**(1./3.) #equivalent volume length and width for each aspect ratio
            widtharr.append(width)  #width is the 'a' axis RADIUS since reqarray values are radii
            length=i*width   #c axis radius
            ...
            poserr_req.append(b1.poserr_req)
            negerr_req.append(b1.negerr_req)
            chreq.append(b1.chreq)
            ...
            poserr_phi.append(b1.poserr_phi)
            negerr_phi.append(b1.negerr_phi)
            ...
            ch_cplx.append(b1.ch_cplx)

            #save_plots = True
            b1.get_characteristics(var ='major_axis', save=save_plots, minor = minor, ch_dist=ch_dist, verbose=False)

            poserr_mjrax.append(b1.poserr_mjrax)
            negerr_mjrax.append(b1.negerr_mjrax)
            ch_majorax.append(b1.ch_majorax)
            min_mjrax.append(b1.min_data)
            max_mjrax.append(b1.max_data)
            mean_mjrax.append(b1.mean_mjrax)

            #save_plots = True
            b1.get_characteristics(var ='depth', save=save_plots, minor = minor, ch_dist=ch_dist, verbose=False)

            poserr_depth.append(b1.poserr_depth)
            negerr_depth.append(b1.negerr_depth)
            ch_depth.append(b1.ch_depth)       
            min_depth.append(b1.min_data)
            max_depth.append(b1.max_data)
            mean_depth.append(b1.mean_depth)
            dphigam.append(b1.dphigam)
            
            if i < 1:
                c_n[phi_ind,req_ind] = b1.ch_depth
                a_n[phi_ind,req_ind] = b1.ch_majorax
                a_avg[phi_ind,req_ind] = b1.mean_mjrax
            else:
                c_n[phi_ind,req_ind] = b1.ch_majorax
                a_n[phi_ind,req_ind] = b1.ch_depth
                a_avg[phi_ind,req_ind] = b1.mean_depth
    ...
    df.to_csv('c_n_lookup.txt', sep='\t')
    df = pd.DataFrame(a_n, index=phio, columns=reqarr)
    #print(df)
    df.to_csv('a_n_lookup.txt', sep='\t')
    df = pd.DataFrame(a_avg, index=phio, columns=reqarr)
    df.to_csv('a_avg_lookup.txt', sep='\t')
    
    return plts.Make_Plots(phio, widtharr, lengtharr, chreq, chphi, ch_ovrlp, ch_majorax, ch_depth, dphigam, 
                           poserr_phi, negerr_phi, poserr_mjrax, negerr_mjrax, poserr_req, negerr_req, poserr_depth,
                           negerr_depth, min_phi, max_phi, min_mjrax, max_mjrax, min_depth, max_depth, min_req, 
                           max_req, mean_phi, mean_mjrax, mean_depth, mean_req, ch_cplx)
...
def sim_clusters(r, length, width, nclusters, ncrystals=2, numaspectratios=1,
    ...
    for n in range(nclusters):
        ...
        while cluster.ncrystals < ncrystals:  
            ...
            if speedy:
                ...
                if plates:
                    rotation = [0, yrotrand, random.uniform(0, 2 * np.pi)]
    ...
    return IceClusterBatch(clusters, length, width, r, numaspectratios, reorient, ovrlps, cplxs, nclusters, plates)
...
def area_overlap(seedcrystal, new_crystal):
    ...
    pctovrlp = (rel_area.area/seedcrystal.projectxy().area)*100
    return(pctovrlp)
...
class IceClusterBatch():
    ...
    def __init__(self, clusters, length, width, r, numaspectratios, reorient, ovrlps, cplxs, nclusters, plates=None): 
        self.length = length
        self.width = width
        ...
        self.poserr_phi = 0.0
        self.negerr_phi = 0.0
        self.poserr_mjrax = 0.0
        self.negerr_mjrax = 0.0
        self.poserr_req = 0.0
        self.negerr_req = 0.0
        self.poserr_depth = 0.0
        self.negerr_depth = 0.0
        self.min_data = 0.0
        self.max_data = 0.0
        self.mean = 0.0
        self.mean_mjrax = 0.0
    ...
    def calc_aspect_ratios(self, minor):
        ...
        if self.plates:
            ...
            self.req[np.isinf(self.req)] = min(self.req)
            self.req[np.isnan(self.req)] = min(self.req)
            #inds = np.where(np.isnan(self.req))
            #self.req[inds] =min(self.req)
    ...
    def get_characteristics(self, var, minor, ch_dist, bins=70, filename=None, ext='png',save = False, verbose=False):
        ...
        if var == 'phi':
            filename = '%.2f_%.1f_%.2f_%.2f' % (self.phio, self.r, self.length, self.width)
            data = self.ratios
            xlabel = "Aggregate Aspect Ratio"
            if ch_dist == 'gamma':
                self.chphi = self.best_fit_distribution(data, ch_dist, xlabel=xlabel, bins=bins, ax=ax)
                #returns characteristic value only from gamma distribution
            else:
                self.chphi = self.best_fit_distribution(data, ch_dist, xlabel=xlabel, bins=bins, ax=ax)
                #returns the characteristic value from the distribution that best fits the data
            self.poserr_phi, self.negerr_phi, self.min_data, self.max_data, self.mean = self.calculate_error(data, self.chphi)
            
            
        if var == 'req':
            filename = 'req_%.3f_%.1f' % (self.phio, self.r)
            data = self.req
            ...
            self.poserr_req, self.negerr_req, self.min_data, self.max_data, self.mean = self.calculate_error(data, self.chreq)


        if var == 'overlap':
            filename = 'overlap_%.3f_%.1f' % (self.phio, self.r)
            data = self.ovrlps
            xlabel = "Overlap"
            if ch_dist == 'gamma':
                self.ch_ovrlp = self.best_fit_distribution(data, ch_dist, xlabel=xlabel, bins=bins, ax=ax)
            else:
                self.ch_ovrlp = self.best_fit_distribution(data, ch_dist, xlabel=xlabel, bins=bins, ax=ax)
            
            
        if var == 'complexity':
            filename = 'cplx_%.3f_%.1f' % (self.phio, self.r)
            data = self.cplxs
            xlabel = "Complexity"
            if ch_dist == 'gamma':
                self.ch_cplx = self.best_fit_distribution(data, ch_dist, xlabel=xlabel, bins=bins, ax=ax)
            else:
                self.ch_cplx = self.best_fit_distribution(data, ch_dist, xlabel=xlabel, bins=bins, ax=ax)
                
        if var == 'depth':
            filename = 'depth_%.3f_%.1f' % (self.phio, self.r)
            data = self.depth           
            ...
            self.poserr_depth, self.negerr_depth, self.min_data, self.max_data, self.mean_depth = \
            self.calculate_error(data, self.ch_depth)
            
            
        if var == 'major_axis':
            filename = 'mjrax_%.3f_%.1f' % (self.phio, self.r)
            ...
            self.poserr_mjrax, self.negerr_mjrax, self.min_data, self.max_data, self.mean_mjrax = \
            self.calculate_error(data, self.ch_majorax)     
