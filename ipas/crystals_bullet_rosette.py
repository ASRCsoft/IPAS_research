...
class IceCrystal:
    ...
    def __init__(self, length, width, center=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        ca = length*2 # c axis radius from equiv. volume radius in lab module
        ...
        #for bullet rosettes, falling orientation does not matter since it is symmetrical
        #therefore, no need to separate point arrays into plates/columns
        self.points = np.array([(x1, mf * f, -mf / 4), (x1, mf * f, mf / 4),
                    (x1, 0, mf / 2), (x1, -mf * f, mf / 4),
                    (x1, -mf * f, -mf / 4), (x1, 0, -mf/2),
                    (-x1, mf * f, -mf / 4), (-x1, mf * f, mf / 4),
                    (-x1, 0, mf / 2), (-x1, -mf * f, mf / 4),
                    (-x1,  -mf * f, -mf / 4), (-x1, 0, -mf/2),
                    ...
                    (-mf / 4, x1, mf * f), (mf / 4, x1, mf * f),
                    (mf / 2, x1, 0), (mf / 4, x1, -mf * f),
                    (-mf / 4, x1, -mf * f), (-mf/2, x1, 0),
                    (-mf / 4, -x1, mf * f), ( mf / 4,  -x1, mf * f),
                    (mf / 2, -x1, 0), (mf / 4, -x1, -mf * f),
                    (-mf / 4, -x1, -mf * f), (-mf/2, -x1, 0)],
    ...
    def plot(self):
        ...
        hex3 = self.points[12:18]
        hex4 = self.points[18:24]
        hex5 = self.points[24:30]
        hex6 = self.points[30:36]
        ...
        # make the lines representing each hexagon
        for hex0 in [hex3, hex4]:
            lines.append(geom.LinearRing(list(hex0)))
            
        # make the lines connecting the two hexagons
        for n in range(6):
            lines.append(geom.LineString([hex3[n], hex4[n]]))
            
        for hex0 in [hex5, hex6]:
            lines.append(geom.LinearRing(list(hex0)))
            
        # make the lines connecting the two hexagons
        for n in range(6):
            lines.append(geom.LineString([hex5[n], hex6[n]]))
...
class IceCluster:
    def __init__(self, crystal, size=1):
        ...
        self.rotation = Quaternion()
        self.points = np.full((size, 36), np.nan,
                              dtype=[('x', float), ('y', float), ('z', float)])
    ...
    def plot_ellipse(self, dims):
        ...
        for l in range(len(dims)):
            ...
            for i in range(self.ncrystals):    
                ...
                hex3pts = self.points[dims[l]][i][12:18]  
                poly3 = geom.Polygon([[p[0], p[1]] for p in hex3pts]) 
                hex4pts = self.points[dims[l]][i][18:24]  
                poly4 = geom.Polygon([[p[0], p[1]] for p in hex4pts])
                hex5pts = self.points[dims[l]][i][24:30] 
                poly5 = geom.Polygon([[p[0], p[1]] for p in hex5pts]) 
                hex6pts = self.points[dims[l]][i][30:36] 
                poly6 = geom.Polygon([[p[0], p[1]] for p in hex6pts])
                ...
                for n in range(7):  #plot the prism face lines
                    x = [x1[n],x2[n]]
                    y = [y1[n],y2[n]]
                    ax.plot(x,y, color = 'navy', zorder = 5, linewidth = '1')
                for n in range(7):  #plot the prism face lines
                    x = [x3[n],x4[n]]
                    y = [y3[n],y4[n]]
                    ax.plot(x,y, color = 'navy', zorder = 5, linewidth = '1')
                ...
                ax.plot(x1,y1, color = 'navy', zorder = 2, linewidth = '2') #edges of polygons
                ax.plot(x2,y2, color = 'navy', zorder = 2, linewidth = '2') 
                ax.plot(x3,y3, color = 'navy', zorder = 2, linewidth = '2') 
                ax.plot(x4,y4, color = 'navy', zorder = 2, linewidth = '2') 
                ax.plot(x5,y5, color = 'navy', zorder = 2, linewidth = '2') 
                ax.plot(x6,y6, color = 'navy', zorder = 2, linewidth = '2') 
