...
def main_ar_loop(phio, reqarr, numaspectratios, ch_dist, nclusters, ncrystals, minor, rand_orient, save_plots, file_ext):    

    #arrays to save the aggregate characteristic variables
    #for plots that vary monomer aspect ratio (plot.py module)
    chreq = []
    chphi = []
    chphi2D = []
    ovrlp = []
    ...
    for i in phio:
        ...
        seed_req_ind = 0
        for s in reqarr:   #array of equivalent volume radii to loop through
            #equivalent volume length and width for each aspect ratio
            #r**3  volume
            seed_width = (s**3/i)**(1./3.)
            seed_widtharr.append(seed_width)  #width is a axis
            seed_length=i*width
            seed_lengtharr.append(length)
            
            new_phi_ind = 0
            for j in phio:               
                
                new_req_ind = 0
                for n in reqarr:   #array of equivalent volume radii to loop through
                    #equivalent volume length and width for each aspect ratio
                    #r**3  volume
                    new_width = (n**3/j)**(1./3.)
                    new_widtharr.append(seed_width)  #width is a axis
                    new_length=j*width
                    new_lengtharr.append(length)
    ...
    for n in range(nclusters):
        ...
        while cluster.ncrystals < ncrystals:  
            ...
            if crystal_hit:
                ...
                ovrlp = overlap(seedcrystal, new_crystal)
...
def overlapXYZ(seedcrystal, new_crystal, cluster, plates):  
    ...
    #calculated as the pixel percentage along each of the three dimensions
    #(e.g.,\ the number of pixels that overlap in the vertical compared to the
    #maximum number of pixels of the aggregate along the z-axis)
    
    #this method was problematic for extreme columns -- little overlap
    #three different overlap measurements (X,Y,Z) that change as the aggregate is rotated
    
    #horizontal overlap
    in_out = ['y','x','z']  #dimension into and out of the page
    dim_up = 'z'   #vertical coordinate
    percent = []    #append overlap percentages
    for i in range(2):
        xmax = cluster.max(in_out[i])   #max x point in each 
...
def complexity(cluster):  #As in Schmitt, Heymsfield 2014
    ...
    circ = make_circle([x[i],y[i]] for i in range(len(x)))  #calls above methods
...
class IceClusterBatch():
    ...
    def calc_aspect_ratios(self, minor):
        ...
        if self.plates:
            ...
            #code to break -- reset to the min value in the data
    ...
    def get_characteristics(self, var, minor, ch_dist, bins=70, filename=None, ext='png',save = False, verbose=False):
        ...
        if var == 'phi':
            ...
            if ch_dist == 'gamma':
                ...
                #smallest sum of square error
            self.poserr_phi, self.negerr_phi, self.min_data, self.max_data, self.mean = self.calculate_error(data, self.chphi)
            #returns mean +/- 1 std. dev and min/max aggregate value for the given data variable
            #same sequence for the other variables below
